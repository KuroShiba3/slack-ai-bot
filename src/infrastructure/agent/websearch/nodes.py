...
"""
    ...
    try:
        ...
        sends = [
            Send("execute_search", {"query": query, "task_id": task_id})
    ...
    try:
        ...
        if task_id and search_results:
            try:
                ...
                results_without_query = [{k: v for k, v in result.items() if k != "query"} for result in search_results]

                db_search_result = SearchResult(
                    task_id=UUID(task_id),
                    query=query,
                    results=results_without_query
                )
                await db_client.save_search_result(db_search_result)
            except Exception as e:
                logger.error(f"検索結果の保存に失敗しました (query: {query}): {e}")
...
4. **情報源の記載形式(必須)**:
    ...
    - **本文中にインライン引用を含める**:
        - 検索結果から得た情報を記述する際、その情報がどの検索結果から来たかを示すために、文中または文末に引用番号を付ける
        - 引用番号の形式: [0] や [1] のように角括弧で囲む
        - 例: 東京の最高気温は15℃です[0]。気象庁によると、平年値は17.3℃です[1]。
        - 複数の検索結果を使用した場合は、それぞれに対応する番号を付ける
...
"""
    ...
    if search_results:
        ...
        for i, result in enumerate(search_results, 1):
            ...
            human_content_parts.append(f"\n**URL**: {url}")
            human_content_parts.append(f"\n**内容**:\n{content}\n")
        human_content_parts.append("\n**【重要】上記のURLを【参考情報】に含める場合は、一字一句完全にコピーしてください。URLの文字を変更・間違えることは絶対禁止です。**")
